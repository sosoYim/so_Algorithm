...
//   function BFS() {
//     // const ch = Array.from({length: 200001},()=>0);
//     // let cow = e;
//     // 레벨, 현수위치
//     // 비교할 소는 레벨 + e
//     // let mvSum = 0;
...
//     while (q.length) {
//       // console.log(q);
//       const [child, cowNow, lv] = q.shift();
//       console.log([child, cowNow, lv]);
//       if (cowNow === 200000 || child === 200000) return -1;
//       if (cowNow === child) return lv;
//       else {
//         const comp = [child + 1, child - 1, child * 2];
//         // mvSum += 1;
//         // cow += mvSum;
//         for (const x of comp) {
//           if (x >= 1 && x <= 200000) {
...
function solution(s, e) {
  ...
  function BFS() {
    ...
    let curX = e; // 소
    while (queue.size) {
      queue = Array.from(queue); // 이건 왜?
      // 송아지가 이동하는 위치를 레벨로 잡고
      // 레벨별로 현수가 이동할 수 있는 모든 경우의 수를 구한다.
      // 같은 레벨에서 현수가 이동하는 경우의 수가 중복되지 않도록 set 에 담는다
      // 업데이트된 queue로 다시 탐색을 시작한다.
      x = x + 1;
      curX += x;
      const next = new Set();
